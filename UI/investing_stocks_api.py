...
class StocksAPI:
    ...
    def getCompanyProfile(self):
        try:
            ...
            profile = investpy.get_stock_company_profile(stock=self.stock, country=self.country, language='english')
            return profile
    ...
    def getFinancialSummary(self,summary_type='income_statement', period='annual'):
        try:
            summary = investpy.get_stock_financial_summary(stock=self.stock, country=self.country, summary_type=summary_type, period=period)
            return summary
    ...
    def getStockData(self):
        try:
            ...
            data = investpy.get_stock_historical_data(stock=self.stock, country=self.country, from_date=self.from_date, to_date=self.to_date, as_json=True)
    ...
    def getRecentStockData(self):
        try:
            data = investpy.get_stock_recent_data(stock=self.stock, country=self.country, as_json=True)
            return data
        except (ValueError, FileNotFoundError, IOError, RuntimeError, ConnectionError):
            if ValueError:
                mb.showinfo('Error','Check parameters for required data. There may be an error or some values missing.')
