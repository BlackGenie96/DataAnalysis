...
class CrossesAPI:
    def __init__(self, cross=None, from_date=None, to_date=None):
        self.cross = cross
    ...
    def getCrossesData(self):
        try:
            data = investpy.get_currency_cross_historical_data(currency_cross=self.cross, from_date=self.from_date, to_date=self.to_date, as_json=True)
            return data
    ...
    def getCrossRecentData(self):
        try:
            data = investpy.get_currency_cross_recent_data(currency_cross=self.cross, as_json=True)
            return data
        except (ValueError, FileNotFoundError, IOError, RuntimeError, ConnectionError):
            if ValueError:
                mb.showinfo('Error','Check parameters for required data. There may be an error or some values missing.')
