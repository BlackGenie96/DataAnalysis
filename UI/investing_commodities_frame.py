...
class PlotCommoditiesFrame(tk.Frame):
    def __init__(self, master):
        ...
        row = tk.Frame(self)
        lab = tk.Label(row, text='Commodities Plotting', font=title_font)
        row.pack()
        lab.pack()

        #combobox for selecting group
        row = tk.Frame(self)
        lab = tk.Label(row, text='Select Group', font=font)
        ...
        row = tk.Frame(self)
        lab = tk.Label(row, text='Select Commodity', font=font)
    ...
    def plotData(self, master):
        ...
        if self.api.getGroup() == None or self.api.getGroup() == "":
            ...
            return
        elif self.api.getCommodity() == None or self.api.getCommodity() == "":
        ...
        data = {
            'commodity_name': self.api.getCommodity(),
        ...
        result = requests.post(f'{server}/investing/commodities/get', data=json.dumps(data), headers=headers).json()
    ...
    def updateData(self):
        #check for bond 
        if self.api.getCommodity() == None or self.api.getCommodity() == "":
            mb.showinfo('Notice', 'Select a commodity')
            return
        ...
        recent = requests.post(f'{server}/investing/commodities/get/recent',data=json.dumps(data),headers=headers)
        print(recent)
        ...
        if recent.status_code == 404: 
            ...
            #get recent data
            recent_data = self.api.getCommodityRecentData()
            print(recent_data)
